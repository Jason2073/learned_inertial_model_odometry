...
def prepare_dataset(args):
    ...
    for idx, seq_name in enumerate(seq_names):
        ...
        separate_thrusts = []  # [ts thrust1 thrust2 thrust3 thrust4]
        dt = 0.01
        n_discarded_rpm = 0
        ...
        separate_thrusts = np.asarray(separate_thrusts)
        ...
        separate_thrusts_tmp = interp1d(separate_thrusts[:, 0], separate_thrusts[:, 1:5], axis=0)(times_imu)
        thrusts = thrusts_tmp
        separate_thrusts = separate_thrusts_tmp
        ...
        separate_thrusts_train = separate_thrusts[idx0_train:idx1_train]
        ...
        separate_thrusts_val = separate_thrusts[idx0_val:idx1_val]
        ...
        separate_thrusts_test = separate_thrusts[idx0_test:idx1_test]
        ...
        with h5py.File(out_fn, "w") as f:
            ...
            traj_target = f.create_dataset("traj_target", data=gt_traj_train[:, 1:])
            ...
            sep_thru = f.create_dataset("full_thrust", data=separate_thrusts_train)
        ...
        if not args.skip_txt:
            ...
            np.savetxt(os.path.join(out_dir, "full_thrust.txt"),
                       np.concatenate((ts_train.reshape((-1, 1)), separate_thrusts_train), axis=1),
                       fmt='%.12f', header='ts thrust1, thrust2, thrust3, thrust4 [N]')
        ...
        with h5py.File(out_fn, "w") as f:
            ...
            traj_target = f.create_dataset("traj_target", data=gt_traj_val[:, 1:])
            traj_target_oris_from_imu_target = \
                f.create_dataset("traj_target_oris_from_imu", data=traj_target_oris_from_imu[:, 1:])
            thru = f.create_dataset("thrust", data=thrusts_val)
            i_thru = f.create_dataset("i_thrust", data=i_thrusts_val)
            sep_thru = f.create_dataset("full_thrust", data=separate_thrusts_val)
            gyro_bias = f.create_dataset("gyro_bias", data=b_g)
            accel_bias = f.create_dataset("accel_bias", data=b_a)
        ...
        if not args.skip_txt:
            ...
            np.savetxt(os.path.join(out_dir, "collective_thrust.txt"),
                       ...
                       fmt='%.12f', header='ts thrust [m/s2]')
            np.savetxt(os.path.join(out_dir, "full_thrust.txt"),
                       np.concatenate((ts_val.reshape((-1, 1)), separate_thrusts_val), axis=1),
                       fmt='%.12f', header='ts thrust1, thrust2, thrust3, thrust4 [N]')
        ...
        with h5py.File(out_fn, "w") as f:
            ...
            traj_target = f.create_dataset("traj_target", data=gt_traj_test[:, 1:])
            traj_target_oris_from_imu_target = \
                f.create_dataset("traj_target_oris_from_imu", data=traj_target_oris_from_imu[:, 1:])
            thru = f.create_dataset("thrust", data=thrusts_test)
            i_thru = f.create_dataset("i_thrust", data=i_thrusts_test)
            sep_thru = f.create_dataset("full_thrust", data=separate_thrusts_test)
            gyro_bias = f.create_dataset("gyro_bias", data=b_g)
            accel_bias = f.create_dataset("accel_bias", data=b_a)
        ...
        if not args.skip_txt:
            ...
            np.savetxt(os.path.join(out_dir, "collective_thrust.txt"),
                       ...
                       fmt='%.12f', header='ts thrust [m/s2]')
            np.savetxt(os.path.join(out_dir, "full_thrust.txt"),
                       np.concatenate((ts_test.reshape((-1, 1)), separate_thrusts_test), axis=1),
                       fmt='%.12f', header='ts thrust1, thrust2, thrust3, thrust4 [N]')
