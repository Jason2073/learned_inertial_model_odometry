...
################################ Loss calculation for Point Cloud ################################
...
def point_cloud_L2_loss(c, c_hat, split):
    ...
    for i in range(samples_per_control-1):
        z1w = R_hat_cat[i,:,:,:,:] @ torch.unsqueeze(x[i,:,:,:], dim=3) + torch.unsqueeze(x_hat_cat[i,:,:,:], dim=3)
        z2b = R_hat_transpose_cat[i+1,:,:,:,:] @ z1w - R_hat_transpose_cat[i+1,:,:,:,:] @ torch.unsqueeze(x_hat_cat[i+1,:,:,:], dim=3)
